# -*- coding: utf-8 -*-
"""
Hardware file for the American Magnetics MAXes system. 
...
class AMIMAxes(Base, MagnetInterface):
    """ Magnet positioning software for AMI MAXes superconducting magnet.
    ...
    aps100:
        module.Class: 'magnet.AMI_MAxes.AMIMAxes'
        magnet_address_x: '192.168.1.17'
        magnet_port_x: 7180

        magnet_address_y: '192.168.1.17'
        magnet_port_y: 7180

        magnet_address_z: '192.168.1.17'
        magnet_port_z: 7180
    ...
    """
# config opts
    addr_x = ConfigOption('magnet_address_x', missing='error')
    port_x = ConfigOption('magnet_port_x', 7180)

    addr_y = ConfigOption('magnet_address_y', missing='error')
    port_y = ConfigOption('magnet_port_y', 7180)

    addr_z = ConfigOption('magnet_address_z', missing='error')
    port_z = ConfigOption('magnet_port_z', 7180)
    ...
    def on_activate(self):
        ...
        """
        self.x_instrument = AMIModel430("ami430_x", address=f"TCPIP0::{self.addr_x}::{self.port_x}")        
        self.y_instrument = AMIModel430("ami430_y", address=f"TCPIP0::{self.addr_y}::{self.port_y}")
        self.z_instrument = AMIModel430("ami430_z", address=f"TCPIP0::{self.addr_z}::{self.port_z}")
    ...
    def script_test(self):
        coil_const = self.x_instrument._coil_constant
        current_rating = self.x_instrument._current_rating
        current_ramp_limit = self.x_instrument._current_ramp_limit
        ...
        print(f"current field = {self.x_instrument.field()} T")
